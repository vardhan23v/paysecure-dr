...
function SeverityBadge({ classification }: { classification: string }) {
...
export default function Runbooks() {
  ...
  const allTags = useMemo(() => {
    ...
    runbookList.forEach((rb) => rb.tags.forEach((t) => tagSet.add(t)));
    return [...tagSet].sort();
  }, []);

  const filtered = useMemo(() => {
    return runbookList.filter((rb) => {
      const matchesSearch =
        !search ||
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {runbookList.length} production-ready DR runbooks
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 opacity-0 animate-fade-in">
        ...
        <p className="mt-3 text-xs text-gray-400 dark:text-gray-500">
          Showing {filtered.length} of {runbookList.length} runbooks
        </p>
      </div>

...
const rpnRanges = [
  { label: 'Critical (≥70)', min: 70, max: 1000, color: 'bg-red-500', count: fmea.filter((f) => f.rpn >= 70).length },
  { label: 'High (40–69)', min: 40, max: 69, color: 'bg-amber-500', count: fmea.filter((f) => f.rpn >= 40 && f.rpn < 70).length },
  { label: 'Medium (<40)', min: 0, max: 39, color: 'bg-emerald-500', count: fmea.filter((f) => f.rpn < 40).length },
];
...
export default function FMEA() {
  ...
  const allComponents = useMemo(() => {
    const set = new Set();
    fmea.forEach((f) => set.add(f.component));
    return [...set].sort();
  }, []);
  ...
  const avgRPN = Math.round(fmea.reduce((sum, f) => sum + f.rpn, 0) / fmea.length);
  const maxRPN = Math.max(...fmea.map((f) => f.rpn));
    ...
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          {fmea.length} failure modes analysed across {allComponents.length} components
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 opacity-0 animate-fade-in" style={{ animationDelay: '0ms' }}>
          ...
          <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{fmea.length}</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 opacity-0 animate-fade-in" style={{ animationDelay: '180ms' }}>
          ...
          <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">
            {new Set(fmea.flatMap((f) => f.runbook.split(', ').map((r) => r.trim()))).size}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 opacity-0 animate-fade-in" style={{ animationDelay: '160ms' }}>
        ...
        <div className="mt-3 w-full bg-gray-100 dark:bg-gray-800 rounded-full h-3 flex overflow-hidden">
          {rpnRanges.map((range) => {
            const pct = (range.count / fmea.length) * 100;
            return pct > 0 ? (
              <div
                key={range.label}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 opacity-0 animate-fade-in" style={{ animationDelay: '200ms' }}>
        ...
        <p className="mt-3 text-xs text-gray-400 dark:text-gray-500">
          Showing {filtered.length} of {fmea.length} failure modes
        </p>
      </div>
